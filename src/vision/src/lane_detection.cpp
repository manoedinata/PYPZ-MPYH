...
class LaneDetection : public rclcpp::Node
{
  public:
    ...
    rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr pub_points_normals;
    ...
    LaneDetection()
        : Node("LaneDetection"), tf_buffer_(this->get_clock()), tf_listener_(tf_buffer_)
    {
        if (!logger.init())
        {
        ...
        pub_points_normals = this->create_publisher<sensor_msgs::msg::PointCloud2>(
            "/detection/points_normals", 1);

        routine_thread_ = std::thread(std::bind(&LaneDetection::callback_routine, this), this);
    ...
    void callback_sub_camera_depth_points(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
    {
...

        {
            ...
            // ne.setInputCloud(points_camera.makeShared());
            ...
            // // Convert normals to PointCloud2 format
            // pcl::PointCloud<pcl::PointXYZ>::Ptr points_with_normals(new pcl::PointCloud<pcl::PointXYZ>());
            // for (size_t i = 0; i < points_camera.size(); ++i) {
            //     pcl::PointXYZ point(points_camera.points[i].x, points_camera.points[i].y, points_camera.points[i].z);
            //     points_with_normals->points.push_back(point);
            // }
            // points_with_normals->width = points_with_normals->points.size();
            // points_with_normals->height = 1; // Set height to 1 for a flat point cloud
            // points_with_normals->is_dense = true;
            // points_with_normals->header = points_camera.header;
            // points_with_normals->header.stamp = pcl_conversions::toPCL(msg->header.stamp);
            // sensor_msgs::msg::PointCloud2 msg_normals;
            // pcl::toROSMsg(*points_with_normals, msg_normals);
            // msg_normals.header = msg->header;
            // msg_normals.header.frame_id = "camera_link"; // transformed frame
            // pub_points_normals->publish(msg_normals);
        }
    ...
    void callback_routine()
    {
        while (rclcpp::ok())
        {
            ...
            //? ==================================================
            {
                ...
                // logger.info("BGR image size: %d x %d", frame_bgr.cols, frame_bgr.rows);
            }
            ...
            //?                   Bird Eye View
            //? ==================================================
            // cv::resize(undistorted_frame, undistorted_frame, cv::Size(640, 480));

            // // Perspective transformation for bird's eye view
            // std::vector<cv::Point2f> src_pts = {
            //     cv::Point2f(undistorted_frame.cols * 0.5 - 85, undistorted_frame.rows * 0.5 - 100),
            //     cv::Point2f(undistorted_frame.cols * 0.5 + 85, undistorted_frame.rows * 0.5 - 100),
            //     // cv::Point2f(undistorted_frame.cols * 0.5 - 370, undistorted_frame.rows * 0.5 - 100),
            //     // cv::Point2f(undistorted_frame.cols * 0.5 + 370, undistorted_frame.rows * 0.5 - 100),
            //     cv::Point2f(0, undistorted_frame.rows),
            //     cv::Point2f(undistorted_frame.cols, undistorted_frame.rows)
            // };

            // std::vector<cv::Point2f> dst_pts = {
            //     cv::Point2f(undistorted_frame.cols * 0.2, 0),
            //     cv::Point2f(undistorted_frame.cols * 0.8, 0),
            //     cv::Point2f(undistorted_frame.cols * 0.2, undistorted_frame.rows),
            //     cv::Point2f(undistorted_frame.cols * 0.8, undistorted_frame.rows)
            // };

            // cv::Mat transformed_frame;
            // cv::Mat M = cv::getPerspectiveTransform(src_pts, dst_pts);
            // cv::warpPerspective(undistorted_frame, transformed_frame, M, undistorted_frame.size());
            //? ==================================================
            //?                 Process Image
            //? ==================================================
            cv::Mat gray_frame;
            ...
            cv::Mat edges;
            cv::Canny(gray_frame, edges, 50, 95, 3);
            ...
            cv::Mat blurred = cv::Mat::zeros(filtered_binary.size(), CV_8UC1);
            cv::GaussianBlur(filtered_binary, blurred, cv::Size(15, 15), 0);
            cv::Mat edges_blurred;
            cv::Canny(blurred, edges_blurred, 50, 95, 3);
            cv::dilate(edges_blurred, edges_blurred, cv::Mat(), cv::Point(-1, -1), 5);
            // for (const auto& contour : contours) {
            //     for (const auto& point : contour) {
            //         if (point.x >= 0 && point.x < edges_blurred.cols && point.y >= 0 && point.y < edges_blurred.rows && edges_blurred.at<uint8_t>(point.y, point.x) > 0) {
            //             point_cloud.push_back(point);
            //         }
            //     }
            // }

            // draw point cloud in debug_frame
            // for (const auto& point : point_cloud) {
            //     cv::circle(debug_frame, cv::Point(point.x + center_cam_x, center_cam_y - point.y), 3, cv::Scalar(255, 255, 255), -1);
            // }
            //? ==================================================
            //?                Convert to Depth
            //? ==================================================
            ...
            //?                   Show Image
            //? ==================================================
            // cv::imshow("Otsu", otsu_binary);
            // cv::imshow("Filtered", filtered_binary);
    ...
    void custom_connected_components(const cv::Mat &binary_image)
    {
        // Manual implementation of connected components
        cv::Mat labels = cv::Mat::zeros(binary_image.size(), CV_32S);
        int label_count = 0;

        // Define the region of interest (ROI)
        int roi_start_y = 0;
        int roi_end_y = binary_image.rows;
        int roi_start_x = 0;
        int roi_end_x = binary_image.cols;

        for (int y = roi_start_y; y < roi_end_y; ++y)
        {
            for (int x = roi_start_x; x < roi_end_x; ++x)
            {
                if (binary_image.at<uint8_t>(y, x) == 255 && labels.at<int>(y, x) == 0)
                {
                    // New component found
                    label_count++;
                    ...
                    while (!stack.empty())
                    {
                        ...
                        // Check bounds including ROI limits
                        if (pt.x < roi_start_x || pt.x >= roi_end_x || pt.y < roi_start_y || pt.y >= roi_end_y)
                            continue;

                        if (labels.at<int>(pt.y, pt.x) != 0 || binary_image.at<uint8_t>(pt.y, pt.x) == 0)
                            continue;

                        labels.at<int>(pt.y, pt.x) = label_count;
                        component.push_back(pt);

                        // Add neighbors (they will be checked against ROI bounds in next iteration)
                        stack.push_back(cv::Point(pt.x + 1, pt.y));
                        stack.push_back(cv::Point(pt.x - 1, pt.y));
                        stack.push_back(cv::Point(pt.x, pt.y + 1));
                        stack.push_back(cv::Point(pt.x, pt.y - 1));
                    }
                }
            }
        ...
        // Display all labels in one frame with different colors
        cv::Mat colored_components = cv::Mat::zeros(binary_image.rows, binary_image.cols, CV_8UC3);

        for (int label = 1; label <= label_count; ++label)
        {
            cv::RNG rng(label * 12345); // Use label to seed random generator for consistent colors
            ...
            for (int y = 0; y < labels.rows; ++y)
            {
                for (int x = 0; x < labels.cols; ++x)
                    if (labels.at<int>(y, x) == label)
                        colored_components.at<cv::Vec3b>(y, x) = cv::Vec3b(color[0], color[1], color[2]);
            }
        }
        // cv::imshow("Connected Components", colored_components);
    }
    ...
    void find_point_cloud(std::vector<cv::Point> *line_ptr, const cv::Mat &mask, uint8_t is_rising)
    {
        ...
        constexpr float angle_step = 1.5f;
        const int num_angles = static_cast<int>(360.0f / angle_step);
